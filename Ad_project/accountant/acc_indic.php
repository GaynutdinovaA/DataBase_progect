...
<body>
<div class="d-flex flex-column flex-shrink-0 p-3 bg-light" style="width: 280px; position: fixed; height: 100%"
     style="border:20px">
...
<div style="margin-left: 350px">
    ...
    <form method="POST" style="">
        ...
        if (empty($_REQUEST['search']))
            ...
            Accountants_Indications.ID_accountant, Indications.Date_indication, Indications.Content_on_the_indication from Indications
            inner join Administrators ON Administrators.ID_administrator=Indications.ID_administrator
            inner join Accountants_Indications ON Accountants_Indications.ID_indication=Indications.ID_indication
                                                                                                                 ORDER BY {$sort_sql}");
        else {
            ...
            Accountants_Indications.ID_accountant, Indications.Date_indication, Indications.Content_on_the_indication from Indications
            inner join Administrators ON Administrators.ID_administrator=Indications.ID_administrator
            inner join Accountants_Indications ON Accountants_Indications.ID_indication=Indications.ID_indication WHERE Indications.Content_on_the_indication like '%$inputSearch%' ORDER BY {$sort_sql}");
        }
        $sth->execute();
